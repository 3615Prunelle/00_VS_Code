...
char	*get_next_line(int fd)
{
	...
	{
		while (free_buffers < AMOUNT_OF_FD)
		{
			free(buffer_array[free_buffers]); //au tout dernier passage, une fois toutes les lignes retournées, free all buffers
			buffer_array[free_buffers] = NULL;
			free_buffers++;
		}
		return (0);
	}
