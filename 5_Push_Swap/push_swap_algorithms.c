...
{
	int index_max = (count_nodes(a_and_b->stack_a));
	int i = 0;
	int bit_mask = 1;
	...
	while(true)
	{
		...
		{
			if(STACK_A_INDEX >= highest_number)
			{
				highest_number = STACK_A_INDEX;
			}

			if(!(STACK_A_INDEX & bit_mask)) // Commencer à transférer les pairs dans stack b (last bit 0)
			{
				PUSH_B;
			}
			else
