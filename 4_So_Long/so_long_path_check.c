...
/*   Updated: 2025/08/21 12:30:59 by sophie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int tot_ctibles)
{
	...
	// tile_char = (game_copy.content)[(player_pos).line][(player_pos).column];
	if (!path_pre_checks(&game_copy, &player_pos))
		return (false);
	if (TILE_CHAR(game_copy.content, player_pos) == COLLECTIBLE)
		ctibles_counter++;
	if (player_pos.line == dest_pos.line && (player_pos.column == dest_pos.column))
		escape_found = 1;
