...
// ------------------------- Deal with the map itself first - Step 1, get the size of it ----------------------------- //
	...
	while (line_by_line != NULL)
	{
		...
		line_counter++;												// 4
	} // ❔ What happens if map not rectangular?
	close(fd);														// Sinon ne lira pas suite
// ------------------------- Step 2, alloc memory & fill it ----------------------------------------------------------- //
	map = ft_calloc((line_counter + 1), sizeof(char *));			// On alloue les lignes uniquement (les colonnes seront allouees par GNL) ‼️ Reminder : Use free or free function later
	if (!map)
	return (NULL);
	fd = open(PATH, O_RDWR);
