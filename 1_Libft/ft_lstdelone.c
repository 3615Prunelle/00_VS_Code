...
/*   Updated: 2025/04/21 13:52:28 by schappuy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/* NOTES :
void (*del)(void *) = Un pointeur vers une fonction qui prend un void * en paramètre et ne retourne rien, donc un pointeur vers la fonction		void	del(void *)
(*del) = variable appelée del qui pointe vers une fonction.
'int *ptr;' >> ptr est un pointeur vers un int || 'void (*del)(...);' >> del est un pointeur vers une fonction
(void *) = pointeur générique en parametre, donc n’importe quel type de donnée.

Concept des callbacks. C’est comme dire :
“Voici du contenu à détruire (node) et la méthode (del) pour effacer son contenu. Peu importe ce que c’est, utilise ce que je t’ai donné pour le gérer.”
ft_lstdelone n’a pas besoin de savoir comment on détruit le contenu. Elle dit juste :
“Passe-moi une fonction qui sait le faire, et je l’appellerai quand il faudra.”
*/
...
// -------------------------------------------- Free the pointer to the node, as it's been malloc'd in ft_lstnew */ ----------------------------------------- //
	if (lst)
	{
// -------------------------------------------- Fetch the data to erase : `lst->content` (lst is a pointer, so I use the -> instead of the .) */ ------------ //
		del(lst->content);
		free(lst);
// -------------------------------------------- Adding this to avoid dangling pointer ----------------------------------------------------------------------- //
		lst = NULL; // this is actually useless, because it doesn't affect the original pointer (because I'm changing a local copy of it). A double ptr would work though
	}
}
...
/* int	main(void)
{
	char *zucchine = "Zucchine";			// no malloc here -> don't use lstclear at the end
...
// -------------------------------------------- Adding a last node that will be deleted ------------------------------------------------------------------- //
	t_list *isidor;
	char *fagioli = ft_strdup("Fagioli");	// malloc (through strdup) on that string, otherwise it will give problem when using free
...
// -------------------------------------------- Freeing the last created node ---------------------------------------------------------------------------- //
	...
	printf ("\n⋅.˳˳.⋅ॱ˙˙ॱ⋅.˳˳.⋅ॱ˙˙ॱᐧ.˳˳.⋅Pass in lstdelone to erase the last node⋅.˳˳.⋅ॱ˙˙ॱ⋅.˳˳.⋅ॱ˙˙ॱᐧ.˳˳.⋅\n\n");
	loop_ptr = alix;
	node = 1;
	while (loop_ptr != NULL)
	{
